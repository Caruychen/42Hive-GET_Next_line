...
/*   Updated: 2021/12/23 16:03:45 by cchen            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	set_buff(t_vec *buff, size_t index)
{
	...
	ft_memcpy(buff->memory, (char *)buff->memory + index + 1,
...
static long	read_line(int fd, t_vec *buff, long *bytes, size_t *offset)
{
	*offset = buff->len * buff->elem_size;
	if (buff->alloc_size - *offset < BUFF_SIZE
		&& ft_vecresize(buff, buff->alloc_size * RESIZE_FACTOR) == -1)
		return (*bytes = -1);
	return (*bytes = read(fd, (char *)buff->memory + *offset, BUFF_SIZE));
}
...
int	get_next_line(const int fd, char **line)
{
	...
	{
		buff->len += (size_t) bytes / buff->elem_size;
		if (ft_memchr((char *)buff->memory + offset, '\n', (size_t) bytes))
			break ;
	}
